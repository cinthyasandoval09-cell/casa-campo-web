...
import { ElenaAI } from './ElenaAI';
import { Product, SaleItem, Sale, Expense, Merma, BoxType, UserProfile, Pedido } from '../types';
...
export function AdminApp({ user, onLogout }: { user: UserProfile; onLogout: () => void }) {
  const [tab, setTab] = useState<'mostrador' | 'inventario' | 'admin' | 'pedidos'>('mostrador');
  ...
  const [unreadPedidos, setUnreadPedidos] = useState(0);
    ...
    try {
      ...
      const u5 = onSnapshot(collection(db, 'pedidos'), (snap) => {
        ...
        snap.forEach(d => {
          ...
          if (data.estado === 'pendiente') newCount++;
        });
        setPedidos(arr.sort((a,b) => b.fecha - a.fecha));
        setUnreadPedidos(newCount);
      });
  ...
  const handleCobrar = async (total: number, ganancia: number, totalGramos: number) => {
    ...
    try {
      ...
      console.error(e);
      showNotification('Error al cobrar: ' + (e.message || 'Intenta de nuevo'));
    }
  };
      ...
      <header className="sticky top-0 z-30 flex-none h-[60px] bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6 shadow-sm">
        ...
        <nav className="flex items-center gap-1">
          {[
            ...
            { id: 'pedidos', label: 'Pedidos', badge: unreadPedidos },
            { id: 'admin', label: 'Admin' }
          ].map(t => (
            <button 
              key={t.id} 
              onClick={() => setTab(t.id as any)}
              className={`relative px-3 py-1.5 rounded-md text-xs font-bold transition-all duration-200 ${tab === t.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
            >
              {t.label}
              {t.badge ? (
                <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] text-white font-black shadow-sm">
                  {t.badge}
                </span>
              ) : null}
            </button>
          ))}
          <button onClick={onLogout} className="px-3 py-1.5 rounded-md text-xs font-bold text-red-600 hover:bg-red-50 ml-2">Salir</button>
        </nav>
      </header>
